...
void goToDeepSleep() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    // Days until next weekday
    int days_to_add = 1 + (current_day / 6) + 2 * ((current_day / 5) - (current_day / 6));
    
    sleep_microseconds = (days_to_add * 24 * 60) - current_total_minutes + target_minutes * 60 * 1000000ULL;
    
    DEBUG_PRINTF("Current: Day %d, %02d:%02d\n", current_day, current_hour, current_minute);
    DEBUG_PRINTF("Sleeping for %d days until next market day %02d:%02d\n", 
                  days_to_add, UPDATE_HOUR, UPDATE_MINUTE);
  } else {

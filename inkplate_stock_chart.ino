...
void setup() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    printStatusLine("Time sync failed");
  }
  ...
  for (int attempt = 0; attempt < ALPHA_VANTAGE_MAX_ATTEMPTS; attempt++) {
    ...
    if (csvData.length() > 0) {
      ...
      if (csvData.indexOf("Error Message") != -1 || csvData.indexOf("Note:") != -1) {
        ...
        printStatusLine("CSV Parser found %d rows", cp.getRowsCount());
...
void goToDeepSleep() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    if (current_minutes < target_minutes) {
      ...
      // Next day
      sleep_minutes = (24 * 60) - current_minutes + target_minutes;
    }
    sleep_microseconds = sleep_minutes * 60 * 1000000ULL;
    esp_sleep_enable_timer_wakeup(sleep_microseconds);
    DEBUG_PRINTF("Sleeping for %d minutes until %02d:%02d\n", 
                  sleep_minutes, UPDATE_HOUR, UPDATE_MINUTE);
  } else {
    // Fallback: sleep for 24 hours
    DEBUG_PRINTLN("Could not get time, sleeping for 24 hours");
  }
